...
export default function Challans() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const [challansResponse, customersResponse, productsResponse] =
                    await Promise.all([
                ...
                if (!challansResponse.ok) {
                    throw new Error(challansData.message || "Failed to load challans");
                }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const challansResponse = await fetch(`${API_URL}/challans`, {
            ...
            if (!challansResponse.ok) {
                throw new Error(
                    challansData.message || "Failed to reload challans"
                );
            }
    ...
    const viewChallan = async (id) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(data.message || "Failed to load challan");
            }
    ...
    const confirmChallan = async (id) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/challans/${id}/confirm`, {
                method: "PUT",
                headers: { Authorization: `Bearer ${token}` }
            });
            const data = await response.json();
            ...
            if (!response.ok) {
                throw new Error(data.message || "Failed to confirm challan");
            }

            setChallans(
                challans.map((challan) =>
                    challan.id === id ? data.challan : challan
                )
            );
    ...
    return (
        <Layout>
            ...
            {showDetails && selectedChallan && (
                <div className="dashboard-card" style={{ marginBottom: "24px" }}>
                    <div className="page-header">
                        <div>
                            <h3>{selectedChallan.challan_number}</h3>
                            <p>Challan Details</p>
                        </div>
                        ...
                        <>
                            <div className="challan-details">
                                <div>
                                    ...
                                    <p>{selectedChallan.customer_name}</p>
                                </div>
                                <div>
                                    <strong>Business</strong>
                                    <p>{selectedChallan.business_name || "—"}</p>
                                </div>
                                <div>
                                    <strong>Status</strong>
                                    <p>{selectedChallan.status}</p>
                                </div>
                                <div>
                                    <strong>Total Quantity</strong>
                                    <p>{selectedChallan.total_quantity}</p>
                                </div>
                            </div>
                            ...
                            {canConfirm && selectedChallan.status === "DRAFT" && (
                                <div className="customer-form-actions">
                                    <button
                                        className="primary-button"
                                        onClick={() => confirmChallan(selectedChallan.id)}
                                    >
                                        Confirm Challan
                                    </button>
                                </div>
                        ...
                        {challans.length === 0 ? (
                            ...
                            challans.map((challan) => (
                                <tr key={challan.id}>
                                    ...
                                    <td>{challan.business_name || "—"}</td>
                                    <td>{challan.total_quantity}</td>
                                    <td>
                                        {challan.status === "DRAFT" ? (
                                            <span className="low-stock">DRAFT</span>
                                        ) : (
                                            <span className="stock-ok">CONFIRMED</span>
                                        )}
                                    </td>
                                    <td>
